...
- `GIT_PREFIX`: git 명령의 prefix. 워크스페이스 모드면 `git -C <worktree-path>`, 일반 모드면 `git`.
  - 워크트리 생성 후에는 `git -C worktrees/<branch-name>`으로 갱신.
- `PROJECT_ROOT`: 프로젝트 소스 코드의 루트 경로. 워크스페이스 모드면 `main/` (워크트리 생성 후 `worktrees/<branch-name>/`), 일반 모드면 현재 디렉토리.
...
1. `DIFF_FILE = ${PROJECT_ROOT}/.dev/diff.txt`. **매 수집 시** `mkdir -p $(dirname ${DIFF_FILE})`을 실행하여 디렉토리 존재를 보장한다 (`--phase review` 단독 실행 등 phase-setup을 거치지 않는 경우 대비).
...
> 3. **워크스페이스 모드**: `git -C main worktree list`로 워크트리를 감지한다.
>    - main 외 워크트리 1개 → 해당 워크트리 사용.
>    - 여러 개 → 사용자에게 선택 (AskUserQuestion).
>    - 없음 → main 사용.
>    - 선택한 워크트리 경로가 존재하지 않으면 `git -C main worktree prune` 실행 후 재감지를 안내한다.
>    - 선택 결과로 `GIT_PREFIX`와 `PROJECT_ROOT`를 갱신한다.
